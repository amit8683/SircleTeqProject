...
function App() {
  ...
  const HandelModifyProduct=()=>{
    const filterData=data.filter((row)=> row.changes==='MODIFIED')
  }

  const getFilteredData = () => {
    return filteredData.filter(row => {
      if (row.type === 'MODIFIED' && !showModified) return false;
      if (row.type === 'PREVIOUS' && !showPrevious) return false;
      if (row.type === 'NEW' && !showNew) return false;
      return true;
    });
  };
  
  const handleModifiedToggle = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4 md:p-8">
      ...
      <div className="text-center mb-6 flex justify-center gap-4">
        ...
        <button  className="px-6 py-3 rounded-lg font-semibold text-white bg-blue-800 hover:bg-green-900"
